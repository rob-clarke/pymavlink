## ECMAScript MAVLink implementation ##

The mavgen_ecma generator generates vanilla ECMAScript implementation of MAVLink. In secure contexts
it has only one dependency: "jpstruct".

This code generates ECMAScript modules packed with `npm` that can be used with Node.js or in the browser.  As with the other implementations in Python and C, the MAVLink protocol is specified in XML manifests which can be modified to add custom messages.

The code here is primarily generate the test suite.

### Generating the JS implementation ###

Folders in the ```implementations/``` directory are `npm` packages containing ECMAScript modules, automatically generated from XML manifests that are in the [mavlink/mavlink](https://github.com/mavlink/mavlink) project.  If you wish to generate custom MAVLink packets, you would need to follow the directions there.
...
let connection = mavlink.MAVLink(null,255,1);

connection.addEventListener('message', (e) => {
	console.log(`Received a message: ${e.detail}`);
	});

let heartbeat = new mavlink.messages.HeartbeatMsg();

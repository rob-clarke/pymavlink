...
var requirestring = '../implementations/mavlink_'+base+'_v'+version+'/mavlink.js';
let mavlinkImport = import(requirestring); //module exports list
console.log("##############################################\n"); 
console.log("Using parser base:"+base+" and version:"+version);

mavlinkImport.then((mavlink) => {

    // create mavlink handler
    let mav = new mavlink.MAVLink();
    ...
    mav.addEventListener('message', function(e) {
    ...
    process.stdin.on('readable', () => {
        ...
        while ((chunk = process.stdin.read()) !== null) {
            ...
            lines.forEach( e => { 
                ...
                var pairs = e.match(/[\dA-F]{2}/gi);
                // convert the octets to integers
                var integers = pairs.map(function(s) {
                    return parseInt(s, 16);
                });
                mav.pushBuffer(new Uint8Array(integers));
                m = mav.parseBuffer();
            } );
        }
    });

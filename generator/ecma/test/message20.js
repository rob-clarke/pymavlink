...
describe('MAVLink 2.0 message registry', function() {
    ...
    beforeEach(function() {
        return new Promise( (beforePromiseResolve) => {
            let mavlinkPromise = import('../implementations/mavlink_ardupilotmega_v2.0/mavlink.js');
...
describe('Complete MAVLink 2.0 packet', function() {
    ...
    it('encode gps_raw_int', function() {
        ...
        var reference = Uint8Array.from([0xfd, 0x1e, 0x00, 0x00, 0x05, 0x2a, 0x96, 0x18, 0x00, 0x00, 0x15, 0xcd, 0x5b, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0c, 0xcf, 0x02, 0x40, 0xf4, 0x7b, 0x00, 0x50, 0xc3, 0x00, 0x00, 0x90, 0x19, 0xd6, 0x11, 0xd3, 0x04, 0xd2, 0x04, 0x03, 0x09, 0x20, 0x2d]);
    ...
    it('encode gps_raw_int with long integer', function() {
        ...
        // Create a buffer that matches what the Python version of MAVLink creates
        var reference = Uint8Array.from([0xfd, 0x1e, 0x00, 0x00, 0x05, 0x2a, 0x96, 0x18, 0x00, 0x00, 0x01, 0x18, 0x3d, 0x9e, 0x59, 0x83, 0xfd, 0x0f, 0x00, 0x0c, 0xcf, 0x02, 0x40, 0xf4, 0x7b, 0x00, 0x50, 0xc3, 0x00, 0x00, 0x90, 0x19, 0xd6, 0x11, 0xd3, 0x04, 0xd2, 0x04, 0x03, 0x09, 0xa2, 0xd3]);
        Uint8Array.from(gpsraw.pack(this.mav)).should.eql(reference);
    });
    ...
    it('encode heartbeat', function() {
        ...
        // Create a buffer that matches what the Python version of MAVLink creates
        var reference = Uint8Array.from([0xfd, 0x09, 0x00, 0x00, 0x07, 0x2a, 0x96, 0x00, 0x00, 0x00, 0x44, 0x00, 0x00, 0x00, 0x05, 0x03, 0x2d, 0x0d, 0x03, 0xa6, 0xe4]);
   ...
   it('encode BATTERY_STATUS, zeroed extensions', function() {
        ...
        var bs = new mavlink.messages.BATTERY_STATUS(
            type=0
            , id=0
            , battery_function=0
            , temperature=32767
        ...
        var reference = Uint8Array.from([0xfd, 0x24, 0x00, 0x00, 0xac, 0x01, 0x01, 0x93, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x7f, 0x2b, 0x31, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x64, 0x15, 0xc3]);

        Uint8Array.from(bs.pack(this.mav)).should.eql(reference);
    });
  ...
  // in mavlink2, this packet has extensions, so being sure they work is useful.
   it('encode BATTERY_STATUS, non-zeroed extensions', function() {
        ...
        var reference = Uint8Array.from([0xfd, 0x29, 0x00, 0x00, 0xac, 0x01, 0x01, 0x93, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x7f, 0x2b, 0x31, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x64, 
 ...
 it('decode BATTERY_STATUS with array, and also zero-d extension bytes', function() {
        ...
        var reference = Uint8Array.from([0xfd, 0x31, 0x00, 0x00, 0xac, 0x01, 0x01, 0x93, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x7f, 0x2b, 0x31, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x64, 
 ...
 it('decode BATTERY_STATUS with array, and also non-zerod extension bytes', function() {
        ...
        var reference = Uint8Array.from([0xfd, 0x29, 0x00, 0x00, 0x58, 0x0b, 0x0a, 0x93, 0x00, 0x00, 0xf8, 0xcd, 0x6d, 0x39, 0xc8, 0xce, 0x6d, 0x39, 0xf3, 0x44, 0x5b, 0x45, 0x5c, 0x45, 0x5d, 0x45, 0x5e, 0x45, 0x5f, 0x45, 0x60, 0x45, 0x61, 0x45, 0x62, 0x45, 0x63, 0x45, 0x64, 0x45, 0x6b, 0x49, 0x65, 0xa8, 0xeb, 0x2e, 0x48, 0xd5, 0x6d, 0x39, 0x7d, 0x90, 0x0a]);
...
describe('MAVLink 2.0 header', function() {
    ...
    it('Can pack itself', function() {
        this.h.pack().should.eql(Uint8Array.from([253, 1, 0, 0, 2, 3, 4, 21, 0, 0]));
    });
...
describe('MAVLink 2.0 message', function() {
    ...
    it('Can pack itself', function() {
        ...
        packed.should.eql(Uint8Array.from([253, 9, 0, 0, 0, 0, 0, mavlink.MAVLINK_MSG_ID.MAVLINK_MSG_ID_HEARTBEAT, 0, 0, // that bit is the header,
    ...
    describe('decode 2.0 function', function() {
        ...
        it('throws an error if the message has a bad prefix', function() {
            var packed = Uint8Array.from([0, 3, 0, 0, 5, 7, 9, 0, 0, 11]); // bad data prefix in header (0, not 253)
            var m = this.m;
            (function() { m.decode(packed); }).should.throw('Invalid MAVLink prefix (0)');
        });

        it('throws an error if the message ID is not known', function() {
            var packed = Uint8Array.from([253, 1, 0, 0, 0, 3, 0, 199, 0, 0, 1, 0, 0]); // 199 = invalid ID
            var m = this.m;
        ...
        it('throws an error if the message length is invalid', function() {
            var packed = Uint8Array.from([253, 3, 0, 0, 257, 0, 0, 0, 0, 0, 0, 0]);
            var m = this.m;

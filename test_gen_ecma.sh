...
# ECMAScript/JavaScript top-level bindings test/s  - there are a few stages of test here..
...
if [[ ! -f "pretests.done.swp" ]]; then
    ...
    printf "${RED}Generating ECMAScript bindings to test...${NC}\n\n"
    sleep 1
    cd generator
    ./gen_js_ecma.sh > $OUT2
    cd ..
    ...
    printf "${RED}Generating C bindings to test ECMAScript against...${NC}\n\n"
    sleep 1
    ...
    #   we quieten them as we really aren't testing THEM here.
    printf "${RED}Compiling C bindings to test ECMAScript against...${NC}\n\n"
    sleep 1
   ...
   printf "${RED}ECMAScript PRETEST setup done.${NC}\n\n"
    touch pretests.done.swp


else

    printf "${RED}ECMAScript PRETEST setup already recently done, skipping.${NC}\n\n"
...
printf "${RED}Running non-NPM ECMAScript create/pack tests ...${NC}\n\n"
sleep 1
node generator/ecma/implementations/mavlink_ardupilotmega_v2.0/mavlink.tests.js > $OUT1
node generator/ecma/implementations/mavlink_ardupilotmega_v1.0/mavlink.tests.js > $OUT1
node generator/ecma/implementations/mavlink_common_v2.0/mavlink.tests.js > $OUT1
node generator/ecma/implementations/mavlink_common_v1.0/mavlink.tests.js > $OUT1
...
printf "\n${RED}ECMAScript Testing done.${NC}\n\n"
